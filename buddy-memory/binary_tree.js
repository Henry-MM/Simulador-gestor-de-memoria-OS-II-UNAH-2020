...
function update_table_diagram()
{
  ...
  for(var i = 0; i < childless_nodes.length; i++)
  {
    ...
    if(node.allocated){
            table_content += "<td width = "+ String( node.value/memorySize*100) +"%  "+ color + " >"
             + label_node + " " + size + 
             "<br><button type='button' onclick='delete_partition(\""+ node.label +"\")'>X</button> </td>";
    }else{
        table_content += "<td width = "+ String( node.value/memorySize*100) +"%  "+ color + " >"
             + label_node + " " + size + " </td>";
    }
